...
namespace density
{
	namespace tests
	{
        ...
        template <typename COMPLETE_ELEMENT, typename... CONSTRUCTION_PARAMS>
            void add_test_case_push_back_by_copy_n_times(
                ContainerTest<TestDenseList<COMPLETE_ELEMENT>> & i_test,
                double i_probability,
                CONSTRUCTION_PARAMS && ... i_construction_parameters )
        {
            i_test.add_test_case("push_back_n_times", [&i_test, &i_construction_parameters...](std::mt19937 & i_random) {
        ...
        void list_test_impl(std::mt19937 & i_random, const char * i_container_name)
		{
...

			{
				...
				//add_test_case_push_back_by_copy_n_times<TestObjectBase>(test, 1., i_random);
