        ...
        */
        ...
        public:
            ...
            template <page_allocation_type ALLOCATION_TYPE>
                static void * try_allocate_page(progress_guarantee const i_progress_guarantee) noexcept
            {
                PageFooter * const new_page = t_thread_entry.template try_allocate_page<ALLOCATION_TYPE>(i_progress_guarantee);
                DENSITY_ASSERT_INTERNAL(get_footer(new_page) == new_page);
                return address_lower_align(new_page, page_alignment);
            }

            template <page_allocation_type ALLOCATION_TYPE>
                static void deallocate_page(void * const i_page) noexcept
            {
                auto * const footer = get_footer(i_page);
                t_thread_entry.template deallocate_page<ALLOCATION_TYPE>(footer);
            }
        ...
        private:
            ...
            public:
                ...
                template <page_allocation_type ALLOCATION_TYPE>
                    PageFooter * try_allocate_page(progress_guarantee i_progress_guarantee) noexcept
                {
                ...
                template <page_allocation_type ALLOCATION_TYPE>
                    void deallocate_page(PageFooter * i_page) noexcept
                {
                    ...
                    do {
                        ...
                        if (m_current_slot->get_stack(ALLOCATION_TYPE).try_push(i_page))
                        {
                            done = true;
                            break;
                        }
                    ...
                    if (!done)
                    {
                        get_private_stack(ALLOCATION_TYPE).push(i_page);
                    }
                }
            ...
            private:
                ...
                static PageFooter * initialize_page(page_allocation_type const i_allocation_type, void * const i_page_mem) noexcept
                {
                    ...
                    if (should_zero)
                    {
                        std::memset(i_page_mem, 0, page_size); // PageFooter is not touched
                    }
