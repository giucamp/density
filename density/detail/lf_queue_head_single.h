...
namespace density
{
    namespace detail
    {
        ...
        template < typename COMMON_TYPE, typename RUNTIME_TYPE, typename ALLOCATOR_TYPE, typename QUEUE_TAIL >
            class LFQueue_Head<COMMON_TYPE, RUNTIME_TYPE, ALLOCATOR_TYPE, concurrency_single, QUEUE_TAIL> : protected QUEUE_TAIL
        {
        private:
            ...
            struct Consume
            {
                ...
                void clean_dead_elements() noexcept
                {
                    ...
                    for (;;)
                    {
                        ...
                        DENSITY_ASSERT_INTERNAL(!ConstConditional(Base::s_needs_end_control) || is_same_page == (prev_control != Base::get_end_control_block(prev_control)));
