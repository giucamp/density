...
namespace density
{
    /** \brief Class template that implements an heterogeneous FIFO container with dynamic size.
        A small_queue_any allocates one monolithic memory buffer with the provided allocator, and sub-allocates
        ...
        \n<b>Exception safeness</b>: Any function of small_queue_any is noexcept or provides the strong exception guarantee.
            @param ELEMENT Base type of the elements of the queue. The queue enforces the compile-time
                ...
                case, the methods of small_queue_any (and its iterators) that returns a pointer to an element
                ...
                that works with any small_queue_any.
                ...
                this case a standard container (like std::queue) instead of std::small_queue_any is a better choice.
        ...
        small_queue_any provides only forward iteration. Only the first element is accessible in constant time (with
        the functions: small_queue_any::front, small_queue_any::begin). The iterator provides access to both the ELEMENT (with
    ...
    template <typename ELEMENT = void, typename ALLOCATOR = std::allocator<ELEMENT>, typename RUNTIME_TYPE = runtime_type<ELEMENT> >
        class small_queue_any final : private std::allocator_traits<ALLOCATOR>::template rebind_alloc<char>
    {
    public:
        ...
        small_queue_any(size_t i_initial_reserved_bytes = 0, size_t i_initial_alignment = 0)
        {
        ...
        small_queue_any(const small_queue_any & i_source)
        ...
        /** Copy assignment. Clear the content of this queue, and copies the content of the source queue (deep copy).
            ...
            \n <b>Exception guarantee</b>: strong (in case of exception the function has no visible side effects).
            \n\b Complexity: linear in the size of the source. */
        small_queue_any & operator = (const small_queue_any & i_source)
        {
            DENSITY_ASSERT(this != &i_source);
            small_queue_any tmp(i_source); // the copy may throw, with this queue still being unmodified
        ...
        public:
            ...
            using reference = typename small_queue_any::reference;
            using const_reference = typename small_queue_any::const_reference;
        ...
        public:
            ...
            using reference = typename small_queue_any::const_reference;
            using const_reference = typename small_queue_any::const_reference;
        ...
        /** Returns the used size in bytes. This size is dependant, in an implementation defined way, to the count, the type and
            ...
            to use the function small_queue_any::empty to check if the queue is empty.
            \remark There is no way to predict if the next push\emplace will cause a reallocation.
        ...
        /** Returns the free size in bytes. This is equivalent to small_queue_any::mem_capacity decreased by small_queue_any::mem_size.
            \remark There is no way to predict if the next push\emplace will cause a reallocation.
    ...
    private:
        detail::QueueImpl<RUNTIME_TYPE> m_impl; /* m_impl manages the memory buffer, but small_queue_any owns it */
    }; // class template small_queue_any
