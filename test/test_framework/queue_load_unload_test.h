...
namespace density_tests
{
    ...
    */
    template <typename QUEUE> class HeterLoadUnloadTest
    {
      public:
        ...
        ~HeterLoadUnloadTest() { delete[] m_id_map; }
        ...
        */
        ...
        {
            i_ostream << "starting queue load unload test with " << i_thread_count
                      << " threads and ";
            ...
            // start the threads
            aligned_vector<ThreadEntry> threads(i_thread_count);
            uint32_t                    thread_index = 0;
            for (auto & thread_entry : threads)
            {
                auto const start_id64 =
                  (static_cast<uint64_t>(thread_index) * id_map_size) / i_thread_count;
            ...
            while (consume_one())
                consumed_by_main++;
            i_ostream << consumed_by_main << " remaining items were consumed by the main thread"
                      << std::endl;

            final_check();

            histogram<size_t> produced_hist("produced by i-th thread"),
              consumed_hist("consumed by i-th thread");
      ...
      private:
        ...
        void thread_run(ThreadEntry & i_entry, uint32_t const i_start_id, uint32_t const i_end_id)
        {
            ...
            while (curr_id < i_end_id)
            {
                ...
                if ((iteration & 1) == 0)
                {
                    ...
                    {
                        i_entry.m_stats.m_produced.fetch_add(
                          temp_produced, std::memory_order_relaxed);
                        i_entry.m_stats.m_consumed.fetch_add(
                          temp_consumed, std::memory_order_relaxed);
                        temp_produced = 0;
                        temp_consumed = 0;
                    }
                }
            ...
            DENSITY_TEST_ASSERT(
              i_entry.m_stats.m_produced.load(std::memory_order_relaxed) == i_end_id - i_start_id);
        }
    ...
    void queue_load_unload_test(
      uint32_t i_thread_count, uint32_t i_produces_per_thread, std::ostream & i_ostream)
    {
