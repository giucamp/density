...
//! [lf_function_queue push example 3]
    ...
    void lf_func_queue_samples(std::ostream & i_ostream)
    {
        ...
        LfFunctionQueueSamples<standard_erasure, mult, mult, seq_cst>::func_queue_samples(
          i_ostream);
        LfFunctionQueueSamples<manual_clear, mult, mult, seq_cst>::func_queue_samples(i_ostream);

        LfFunctionQueueSamples<standard_erasure, single, mult, seq_cst>::func_queue_samples(
          i_ostream);
        LfFunctionQueueSamples<manual_clear, single, mult, seq_cst>::func_queue_samples(i_ostream);

        LfFunctionQueueSamples<standard_erasure, mult, single, seq_cst>::func_queue_samples(
          i_ostream);
        LfFunctionQueueSamples<manual_clear, mult, single, seq_cst>::func_queue_samples(i_ostream);

        LfFunctionQueueSamples<standard_erasure, single, single, seq_cst>::func_queue_samples(
          i_ostream);
        LfFunctionQueueSamples<manual_clear, single, single, seq_cst>::func_queue_samples(
          i_ostream);

        LfFunctionQueueSamples<standard_erasure, mult, mult, relaxed>::func_queue_samples(
          i_ostream);
        LfFunctionQueueSamples<manual_clear, mult, mult, relaxed>::func_queue_samples(i_ostream);

        LfFunctionQueueSamples<standard_erasure, single, mult, relaxed>::func_queue_samples(
          i_ostream);
        LfFunctionQueueSamples<manual_clear, single, mult, relaxed>::func_queue_samples(i_ostream);

        LfFunctionQueueSamples<standard_erasure, mult, single, relaxed>::func_queue_samples(
          i_ostream);
        LfFunctionQueueSamples<manual_clear, mult, single, relaxed>::func_queue_samples(i_ostream);

        LfFunctionQueueSamples<standard_erasure, single, single, relaxed>::func_queue_samples(
          i_ostream);
        LfFunctionQueueSamples<manual_clear, single, single, relaxed>::func_queue_samples(
          i_ostream);
    }
