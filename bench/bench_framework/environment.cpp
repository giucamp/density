...
#define TESTITY_COM_CALL(call)                                                                     \
    {                                                                                              \
        if (FAILED((call)))                                                                        \
            throw std::exception("Com call failed: " #call);                                       \
    }
    ...
    std::string ConvertWCSToMBS(const wchar_t * pstr, long wslen)
    {
        ...
        len = ::WideCharToMultiByte(
          ...
          0 /* no flags */,
          pstr,
          wslen /* not necessary NULL-terminated */,
          &dblstr[0],
          len,
          NULL,
    ...
    class WMIServices
    {
      public:
        WMIServices()
        {
            TESTITY_COM_CALL(CoCreateInstance(
            ...
            TESTITY_COM_CALL(m_locator->ConnectServer(
              L"root\\cimv2",
              NULL,
              NULL,
              NULL,
        ...
        std::vector<CComPtr<IWbemClassObject>> execute_query(const wchar_t * i_query)
        {
            ...
            TESTITY_COM_CALL(
              m_services->ExecQuery(L"WQL", query, WBEM_FLAG_FORWARD_ONLY, NULL, &enum_object));
